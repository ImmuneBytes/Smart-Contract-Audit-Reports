...
const WOWSERC1155 = artifacts.require('WOWSERC1155');

contract('WOWSERC1155', function (accounts) {
  const [ deployer, other ] = accounts;

  const firstTokenId = new BN('845');
  const firstTokenIdAmount = new BN('1');

  const secondTokenId = new BN('48324');
  const secondTokenIdAmount = new BN('1');
  ...
  beforeEach(async function () {
    this.token = await WOWSERC1155.new(deployer, ZERO_ADDRESS, uri, uri, { from: deployer });
  });
  ...
  describe('burning', function () {
    it('holders can burn their tokens', async function () {
      ...
      const receipt = await this.token.burn(other, firstTokenId, firstTokenIdAmount, { from: other });
      expectEvent(receipt, 'TransferSingle',
        { operator: other, from: other, to: ZERO_ADDRESS, value: firstTokenIdAmount, id: firstTokenId },
      );

      expect(await this.token.balanceOf(other, firstTokenId)).to.be.bignumber.equal('0');
    });
  });
});
