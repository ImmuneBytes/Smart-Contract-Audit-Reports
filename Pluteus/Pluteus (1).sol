/**
   #Features:
    - fee 9% to be splitted:
        - 4% reflection back to holders
        - 4% - to be locked at LP
        - 1% - to be sent to multisig wallet and creates PluteusFund financing charity projects and further development
	- transactions where wallet size would exceed 1% of total supply is declined
	- 48 hours sanity check delay on outgoing transactions from PluteusFund and LP
*/
...
contract Pluteus is Context, IERC20, Ownable, ITimelockable {
    ...
    uint256 private _tTotal = 1 * 10**12 * 10**9; // one trillion 
    ...
    uint256 public _taxFee = 4;  // holders fee
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _liquidityFee = 4;    // liqudity fee: 2% BNB and 2% as pancake LP
    uint256 private _previousLiquidityFee = _liquidityFee;
    
    uint256 public   _pluteusFund = 1;        // pluteus funds
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(address recievedBy) {
        if (sender == PLUTEUS_ADDRESS) { // Lock tokens outgoing of PLUTEUS FUND for 2 days
            require(!isContract(recipient), "Pluteus funds can only be cashed out to EOAs"); // Prevent Timelock bypass
